...
function CombatJobCalculator() {
    ...
    const calculateCombat = () => {
        ...
        for (let i = x1; i < x2; i++) {
            experienceNeeded += calculateFormula(i, jobType);
        }
        ...
        const results = combatMobs.map(mob => {
            ...
            let killCount = Math.ceil(experienceNeeded / effectiveExp);
            let questCount = 0;
            ...
            if (mob.hasQuest && mob.questCount) {
                ...
                let totalExp = 0;
                let kills = 0;
                while (totalExp < experienceNeeded) {
                    kills++;
                    totalExp += effectiveExp;
                    if (kills % mob.questCount === 0) {
                        totalExp += totalQuestExp;
                        questCount++;
                    }
                }
                killCount = kills;
...
(1 湧き当たり)`;
    ...
    const calculateDelivery = () => {
        ...
        deliveryItems.forEach(item => {
            const count = Math.ceil(experienceNeeded / item.exp);
            results[item.name] = {
                count,
        ...
        if (!isNaN(xp) && xp > 0) {
            specifiedXPTotal = Math.ceil(experienceNeeded / xp);
        }
